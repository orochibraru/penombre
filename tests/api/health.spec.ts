...
test.describe("API Health and Basic Functionality", () => {
    test("health endpoint responds correctly", async ({ request }) => {
        ...
        // If any service is unavailable, expect 500
        if (data.DB === "unavailable" || data.Storage === "unavailable") {
...
test.describe("File Operations API", () => {
    ...
    test("upload endpoint exists and returns proper error for unauthenticated request", async ({
        request,
    }) => {
...
test.describe("Authentication API", () => {
    test("auth endpoints exist", async ({ request }) => {
        ...
        expect(
            [200, 401, 404, 405].includes(loginResponse.status())
        ).toBeTruthy();
    });
    ...
    test("logout endpoint exists", async ({ request }) => {
        ...
        expect(
            [200, 401, 404, 405].includes(logoutResponse.status())
        ).toBeTruthy();
    });
});
