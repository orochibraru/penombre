...
CREATE UNIQUE INDEX "files_live_path_idx" ON "files" USING btree ("owner_id",coalesce("volume_id", ''),"path") WHERE is_trashed = false;--> statement-breakpoint
CREATE UNIQUE INDEX "folders_live_path_idx" ON "folders" USING btree ("owner_id",coalesce("volume_id", ''),"path") WHERE is_trashed = false;