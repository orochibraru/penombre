...
export interface paths {
	...
	"/api/v1/client-errors": {
		 ...
		 * @description Logs an unexpected error thrown in the browser under the id the error page shows. Does not require authentication, so a crash on the sign-in page is reported too.
		 */
		post: {
			parameters: {
				query?: never;
	...
	"/api/v1/storage/file/{id}/office": {
		...
		post: {
			...
			requestBody: {
				content: {
					"application/json": {
						/** @description The edited text, in the format for its kind */
						content: string;
					};
				};
			};
	...
	"/api/v1/storage/file/{id}/versions/extract": {
		...
		put?: never;
		/**
		 * Turn versions back into files
		 * @description The reverse of a merge: each version leaves the history and becomes a file beside this one, under its original name made unique, dated by its own bytes. `ids` picks versions; absent takes them all.
		 */
		post: {
			parameters: {
				query?: {
	...
	"/api/v1/storage/file/{id}/versions/zip": {
		parameters: {
			query?: never;
			header?: never;
			path?: never;
		...
		/**
		 * Download every version as a zip
		 * @description The history oldest first, then the current file, numbered so they list in that order: `01 - Song-001.wav` … `04 - Song.wav`.
		 */
		get: {
			parameters: {
				query?: {
	...
	"/api/v1/storage/trash": {
		...
		/**
		 * Empty the trash
		 * @description Permanently deletes every trashed file and folder, and reports what was freed
		 */
		delete: {
			parameters: {
				query?: {
