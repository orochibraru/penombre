...
import { isRedisAvailable } from "#lib/server/cache/test-helpers.js";
import { CacheKeys, CacheManager } from "#lib/server/services/storage/cache.js";
...
describe("MemoryCacheBackend", () => {
	...
	test("returns undefined for expired entries", async () => {
		const cache = createCache();
		await cache.set("key1", "value", 1); // 1 second TTL
		jest.setSystemTime(BASE_TIME + 1001); // 1 ms past expiry
