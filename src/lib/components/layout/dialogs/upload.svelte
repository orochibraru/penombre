<script lang="ts">
	...
	import type { BarebonesFile, UploadResult } from '$lib/server/services/storage';
	...
	async function resultCallback(results: FullResult[]) {
		for (const result of results) {
			try {
				...
				await promise.finally(async () => {
					const { data: file } = await api.v1.storage.objects.object.get({
						query: { key: fileNameWithoutFolder(result.data.finalName), folder: page.params.path }
					});
	...
	async function handleUpload() {
		...
		for (const file of $files) {
			...
			let metadata: BarebonesFile['metadata'] | null = null;
			...
			const promise = api.v1.storage.objects
				...
				.then((res) => {
					...
					const result = { data: res.data, file };
	...
	$effect(() => {
		if (Object.keys($uploadingItems).length > 0) {
			window.onbeforeunload = () => 'Some files are still uploading.';
		} else {
			// Clear the handler when no uploads are in progress
			window.onbeforeunload = null;
		}

		// Cleanup function to run when the effect is re-run or the component is unmounted
		return () => {
			window.onbeforeunload = null;
		};
	});
