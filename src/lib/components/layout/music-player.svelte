<script lang="ts">
	...
	onMount(() => {
      ...
      `;
		...
		const playerEl = document.querySelector('#media-theme-tailwind-audio');
		if (playerEl) {
			// @ts-ignore
			playerEl.template = template;
		}
	});
</script>

<media-theme id="media-theme-tailwind-audio" style="--media-primary-color: #0062ff; width: 100%;">
	<audio slot="media" src={sourceUrl} playsinline autoplay crossorigin="anonymous"></audio>
</media-theme>
