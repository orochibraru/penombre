...
export interface ItemAction {
	...
	fileOnly?: boolean;
	folderOnly?: boolean;
	/** Checked each time the menu renders, for the item it is shown for. */
	hidden?: (item: ObjectItem) => boolean;
...
export interface SharedFileDisplayProps {
	...
	/** Set by a server-paginated listing: the list already arrives in order,
	 *  so re-sorting client-side would only be correct within the loaded
	 *  window and wrong across a page boundary. */
	preSorted?: boolean;
