...
Status: Phase 1 implemented (see "Changes made while implementing").
...
- **Every driver is wrapped, reads always sniff.** The plan wrapped only when
  `encrypt` was set, so a volume whose `_ENCRYPT` was turned off would have
  served its sealed files as ciphertext. `encrypt` now only decides writes.
- **Rewrap is a staged copy, not an in-place header write.** A torn 76-byte
  write at offset 0 destroys the wrapped file key, and with it the file. Still
  no re-encryption: the chunks are copied as is.
- **The sweep also visits mounted volumes, rewrap only, and only while a retired
  key is loaded.** Otherwise what `_ENCRYPT` sealed on a volume stayed on the
  old key forever, and removing `ENCRYPTION_KEY_PREVIOUS` when the log said so
  made it unreadable. Plaintext on a volume is still never touched (`rewrapOnly`
  in the `encrypt` spec).
- **Every open authenticates the last chunk first.** The last-chunk flag only
  detects truncation if something reads the last chunk; a range read of the
  start of a cut file would otherwise succeed. Costs one extra chunk per open.
- **Thumbnails render to memory.** ffmpeg writes webp to `pipe:1`, pdftoppm
  reads stdin and writes PNG to stdout: no plaintext temp file beside sealed
  data, sealed or not. Verified locally with poppler 26.08; Alpine's poppler
  still to check (see risks).
- **Vectors:** the 0 B and 1 B cases carry full hex; the 64 KiB cases carry
  their SHA-256 and length, to keep the fixture small. Generated by the Go test
  (`PENOMBRE_WRITE_VECTORS=1`), reproduced independently by the TS test.
- **`keyring()` / `encryptionEnabled()` live in `crypto/keyring.ts`**, not
  `config.ts`: the unit tests' global config mock would otherwise need every new
  export, in three places. The parsing itself is in `config.ts` as planned.
- **Worker startup warning** reads `app_settings.encryptionKeyId` from the jobs
  database; a worker cannot otherwise know whether the app encrypts.
- **`streamAndCleanUp` is async**, since it sniffs the archive first.
- **Stage files:** the driver stages as `.<name>.<uuid>.tmp` (dot-named, so the
  scan and the sweep skip it); the sweep also skips `<name>.<uuid>.tmp`.
- **Not done:** a deterministic test of the sweep's stat guard (no hook to
  inject a write between the re-stat and the rename without an exported test
  seam); the E2E stacks were wired but not run.
