...
/**
 * Service for creating bulk downloads (zip archives) from storage.
 * Handles display name resolution, directory walking, and streaming archive creation.
 */
export class BulkDownloadService {
	 ...
	 */
	private async getDisplayName(fullPath: string): Promise<string> {
	...
	private async walkDirectoryWithNames(
		...
		for (const entry of entries) {
			...
			if (entry.isDirectory()) {
				...
				const displayName = await this.getDisplayName(fullPath);
				results.push({
					fullPath,
