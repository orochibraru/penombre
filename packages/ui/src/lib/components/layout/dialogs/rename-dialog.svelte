<script lang="ts">
	...
	async function handleRename() {
		...
		const promise = api
			.PUT('/api/v1/storage/objects/item', {
				...
				body: {
					type: $itemAction.item.contentType || 'application/octet-stream',
					key: finalName,
	...
	let open = $derived(($itemAction && $itemAction.open) || false);
...
<Dialog.Root bind:open>
	<Dialog.Content>
		<Dialog.Header>
			<Dialog.Title>Rename {$itemAction.item?.key ?? 'this file'}</Dialog.Title>
			<Dialog.Description>This will change the name of this item.</Dialog.Description>
		</Dialog.Header>
		<form
			onsubmit={async (e) => {
				e.preventDefault();
		...
		>
			...
			<Dialog.Footer>
				<Button onclick={() => ($itemAction.open = false)} variant="outline" type="button">
					Cancel
				</Button>
