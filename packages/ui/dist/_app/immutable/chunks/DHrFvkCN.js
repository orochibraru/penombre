...
class e {
	static fromBin(t, a = 0) {
		return new e(e.decode(t, a));
	}
	static decode(t, a = 0) {
		const r = new DataView(t.buffer, a);
		return `${r.getUint32(0, !0).toString(16)}-${r.getUint16(4, !0).toString(16)}-${r.getUint16(6, !0).toString(16)}-${r.getUint16(8).toString(16)}-${H(t.slice(a + 10, a + 16))}`.toUpperCase();
	}
	...
	static encode(t) {
		...
		return (
			r.setUint32(0, Number.parseInt(t.slice(0, 8), 16), !0),
			r.setUint16(4, Number.parseInt(t.slice(9, 13), 16), !0),
			r.setUint16(6, Number.parseInt(t.slice(14, 18), 16), !0),
			a.set(P(t.slice(19, 23)), 8),
			a.set(P(t.slice(24)), 10),
			a
		);
	}
	constructor(t) {
...
const Z = {
	...
	g = {
		len: 24,
		get: (n, t) => ({ objectId: e.fromBin(n, t), objectSize: Number(o.get(n, t + 16)) })
	};
class l {
	constructor(t) {
		this.len = Number(t.objectSize) - g.len;
	}
	postProcessTag(t, a, r, s) {
		if (a === 'WM/Picture') t.push({ id: a, value: I.fromBuffer(s) });
		else {
			const c = R(r);
			if (!c) throw new L(`unexpected value headerType: ${r}`);
			t.push({ id: a, value: c(s) });
		}
	}
}
class U extends l {
...
class O extends l {
	get(t, a) {
		return {
			fileId: e.fromBin(t, a),
			fileSize: o.get(t, a + 16),
			creationDate: o.get(t, a + 24),
			dataPacketsCount: o.get(t, a + 32),
			playDuration: o.get(t, a + 40),
			sendDuration: o.get(t, a + 48),
			preroll: o.get(t, a + 56),
			flags: { broadcast: A(t, a + 64, 24), seekable: A(t, a + 64, 25) },
			minimumDataPacketSize: b.get(t, a + 68),
			maximumDataPacketSize: b.get(t, a + 72),
			maximumBitrate: b.get(t, a + 76)
		};
	}
}
...
class T extends l {
	get(t, a) {
		return {
			streamType: e.decodeMediaType(e.fromBin(t, a)),
			errorCorrectionType: e.fromBin(t, a + 8)
		};
	}
}
T.guid = e.StreamPropertiesObject;
class k {
	constructor() {
		this.len = 22;
	}
	get(t, a) {
		const r = new DataView(t.buffer, a);
		return {
			reserved1: e.fromBin(t, a),
			reserved2: r.getUint16(16, !0),
			extensionDataSize: r.getUint16(18, !0)
		};
	}
}
k.guid = e.HeaderExtensionObject;
const G = { len: 20, get: (n, t) => ({ entryCount: new DataView(n.buffer, t).getUint16(16, !0) }) };
async function v(n) {
	const t = await n.readNumber(E);
	return (await n.readToken(new h(t * 2, 'utf-16le'))).replace('\0', '');
}
async function f(n) {
	const t = await n.readToken(G),
		a = [];
	for (let r = 0; r < t.entryCount; ++r) a.push(await te(n));
	return a;
}
async function ee(n) {
	const t = await n.readNumber(E),
		a = new Uint8Array(t);
	return (await n.readBuffer(a), a);
}
async function te(n) {
	const t = await n.readNumber(E);
	return {
		type: { videoCodec: (t & 1) === 1, audioCodec: (t & 2) === 2 },
		codecName: await v(n),
...
class w extends l {
	get(t, a) {
		const r = [],
			s = new DataView(t.buffer, a);
		let c = 10;
		for (let i = 0; i < w.contentDescTags.length; ++i) {
			const d = s.getUint16(i * 2, !0);
			if (d > 0) {
				const u = w.contentDescTags[i],
					C = c + d;
				(r.push({ id: u, value: p(t.slice(a + c, a + C)) }), (c = C));
			}
		}
		return r;
	}
}
w.guid = e.ContentDescriptionObject;
w.contentDescTags = ['Title', 'Author', 'Copyright', 'Description', 'Rating'];
class x extends l {
	get(t, a) {
		const r = [],
			s = new DataView(t.buffer, a),
		...
		for (let d = 0; d < c; d += 1) {
			...
			i += 2;
			const C = p(t.slice(a + i, a + i + u));
			i += u;
			const m = s.getUint16(i, !0);
			i += 2;
			const D = s.getUint16(i, !0);
			i += 2;
			const j = t.slice(a + i, a + i + D);
			((i += D), this.postProcessTag(r, C, m, j));
		}
		return r;
	}
}
...
class y extends l {
	get(t, a) {
		...
		return {
			...
			flags: {
				reliableFlag: A(t, a + 40, 0),
				seekableFlag: A(t, a + 40, 1),
				resendLiveCleanpointsFlag: A(t, a + 40, 2)
			},
...
y.guid = e.ExtendedStreamPropertiesObject;
class F extends l {
	get(t, a) {
		const r = [],
			s = new DataView(t.buffer, a),
		...
		for (let d = 0; d < c; d += 1) {
			...
			i += 4;
			const D = p(t.slice(a + i, a + i + u));
			i += u;
			const j = t.slice(a + i, a + i + m);
			((i += m), this.postProcessTag(r, D, C, j));
		}
		return r;
	}
}
F.guid = e.MetadataObject;
class S extends F {}
S.guid = e.MetadataLibraryObject;
class I {
	static fromBuffer(t) {
	...
	get(t, a) {
		const r = new DataView(t.buffer, a),
			...
			c = r.getInt32(1, !0);
		let i = 5;
		for (; r.getUint16(i) !== 0; ) i += 2;
		const d = new h(i - 5, 'utf-16le').get(t, 5);
		for (; r.getUint16(i) !== 0; ) i += 2;
		const u = new h(i - 5, 'utf-16le').get(t, 5);
		return { type: _[s], format: d, description: u, size: c, data: t.slice(i + 4) };
	}
}
const B = J('music-metadata:parser:ASF'),
	ae = 'asf';
class ne extends q {
	async parse() {
		const t = await this.tokenizer.readToken(Z);
		if (!t.objectId.equals(e.HeaderObject))
			throw new L(`expected asf header; but was not found; got: ${t.objectId.str}`);
		try {
			await this.parseObjectHeader(t.numberOfHeaderObjects);
		} catch (a) {
			B('Error while parsing ASF: %s', a);
		}
	}
	async parseObjectHeader(t) {
		...
		do {
			...
			switch ((B('header GUID=%s', r.objectId.str), r.objectId.str)) {
				case O.guid.str: {
					const s = await this.tokenizer.readToken(new O(r));
					(this.metadata.setFormat(
						'duration',
						Number(s.playDuration / BigInt(1e3)) / 1e4 - Number(s.preroll) / 1e3
					),
						this.metadata.setFormat('bitrate', s.maximumBitrate));
					break;
				}
				case T.guid.str: {
					const s = await this.tokenizer.readToken(new T(r));
					this.metadata.setFormat('container', `ASF/${s.streamType}`);
					break;
				}
				case k.guid.str: {
					const s = await this.tokenizer.readToken(new k());
					await this.parseExtensionObject(s.extensionDataSize);
					break;
				}
				case w.guid.str:
					((a = await this.tokenizer.readToken(new w(r))), await this.addTags(a));
					break;
				case x.guid.str:
					((a = await this.tokenizer.readToken(new x(r))), await this.addTags(a));
					break;
				case e.CodecListObject.str: {
					const s = await f(this.tokenizer);
					s.forEach((i) => {
				...
				case e.PaddingObject.str:
					(B('Padding: %s bytes', r.objectSize - g.len),
						await this.tokenizer.ignore(r.objectSize - g.len));
					break;
				default:
					(this.metadata.addWarning(`Ignore ASF-Object-GUID: ${r.objectId.str}`),
						B('Ignore ASF-Object-GUID: %s', r.objectId.str),
						await this.tokenizer.readToken(new U(r)));
			}
		} while (--t);
	}
	async addTags(t) {
		await Promise.all(t.map(({ id: a, value: r }) => this.metadata.addTag(ae, a, r)));
	}
	async parseExtensionObject(t) {
		do {
			const a = await this.tokenizer.readToken(g),
				r = a.objectSize - g.len;
			switch (a.objectId.str) {
				case y.guid.str:
					await this.tokenizer.readToken(new y(a));
					break;
				case F.guid.str: {
					const s = await this.tokenizer.readToken(new F(a));
					await this.addTags(s);
					break;
				}
				case S.guid.str: {
					const s = await this.tokenizer.readToken(new S(a));
					await this.addTags(s);
					break;
				}
				case e.PaddingObject.str:
				...
				case e.ASF_Index_Placeholder_Object.str:
					...
					break;
				default:
					(this.metadata.addWarning(`Ignore ASF-Object-GUID: ${a.objectId.str}`),
						await this.tokenizer.readToken(new U(a)));
					break;
			}
			t -= a.objectSize;
		} while (t > 0);
