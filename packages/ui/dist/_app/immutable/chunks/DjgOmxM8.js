...
const l = c('music-metadata:parser:musepack:sv8'),
	x = new w(2, 'latin1'),
	u = { len: 5, get: (a, e) => ({ crc: i.get(a, e), streamVersion: F.get(a, e + 4) }) },
	p = {
		len: 2,
		get: (a, e) => ({
...
class C {
	...
	async readVariableSizeField(e = 1, t = 0) {
		...
		return (s & 128) === 0
			? { len: e, value: t + s }
			: ((s &= 127), (s += t), this.readVariableSizeField(e + 1, s << 7));
	}
}
class h extends L('Musepack') {}
const M = c('music-metadata:parser:musepack');
class T extends y {
	constructor() {
		(super(...arguments), (this.audioLength = 0));
	}
	async parse() {
		if ((await this.tokenizer.readToken(P)) !== 'MPCK') throw new h('Invalid Magic number');
		return (this.metadata.setFormat('container', 'Musepack, SV8'), this.parsePacket());
	}
	async parsePacket() {
		const e = new C(this.tokenizer);
		do {
			const t = await e.readPacketHeader();
			switch ((M(`packet-header key=${t.key}, payloadLength=${t.payloadLength}`), t.key)) {
				case 'SH': {
					const s = await e.readStreamHeader(t.payloadLength);
					(this.metadata.setFormat('numberOfSamples', s.sampleCount),
						this.metadata.setFormat('sampleRate', s.sampleFrequency),
						this.metadata.setFormat('duration', s.sampleCount / s.sampleFrequency),
						this.metadata.setFormat('numberOfChannels', s.channelCount));
					break;
				}
				case 'AP':
				...
				case 'SE':
					return (
						...
						S(this.metadata, this.tokenizer, this.options)
					);
				default:
...
class V {
	...
	async read(e) {
		for (; this.dword === null; ) this.dword = await this.tokenizer.readToken(i);
		let t = this.dword;
		return (
			(this.pos += e),
			this.pos < 32
...
const b = {
		...
		get: (a, e) => {
			const t = {
				signature: new TextDecoder('latin1').decode(a.subarray(e, e + 3)),
...
class H extends y {
	...
	async parse() {
		const e = await this.tokenizer.readToken(b);
		if (e.signature !== 'MP+') throw new h('Unexpected magic number');
		(k(`stream-version=${e.streamMajorVersion}.${e.streamMinorVersion}`),
		...
		(this.metadata.setFormat('numberOfSamples', t),
			...
			(this.bitreader = new V(this.tokenizer)),
			this.metadata.setFormat('numberOfChannels', e.midSideStereo || e.intensityStereo ? 2 : 1));
		const s = await this.bitreader.read(8);
		return (
			this.metadata.setFormat('codec', (s / 100).toFixed(2)),
			await this.skipAudioData(e.frameCount),
			k(`End of audio stream, switching to APEv2, offset=${this.tokenizer.position}`),
			S(this.metadata, this.tokenizer, this.options)
		);
	}
...
class A extends v {
	async postId3v2Parse() {
		...
		switch (e) {
			case 'MP+': {
				(g('Stream-version 7'), (t = new H(this.metadata, this.tokenizer, this.options)));
				break;
			}
			case 'MPC': {
				(g('Stream-version 8'), (t = new T(this.metadata, this.tokenizer, this.options)));
				break;
			}
			default:
