---
title: Beszel
description: Set up Opendrive authentication for Beszel
---
...
1. Create a new OIDC Client in Opendrive (e.g., `beszel`).
...
1. Open the Opendrive settings page and navigate to **`Application Configuration`**.
...
> Make sure **"Emails Verified"** is enabled in Opendrive or that your identity provider returns a verified email in its `userinfo` response.
...
8. Fill in the required fields with values from Opendrive:
   - **Client ID**
   - **Client Secret**
