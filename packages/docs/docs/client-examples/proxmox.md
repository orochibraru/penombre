---
title: Proxmox
description: Configure Proxmox VE with Opendrive OIDC
---
...
1. In Opendrive create a new OIDC Client, name it, for example, `Proxmox`.
...
5. Paste the `Client ID` from Opendrive into the `Client ID` field in Proxmox.
6. Paste the `Client Secret` from Opendrive into the `Client Key` field in Proxmox.
...
11. Sign in to Proxmox with a Opendrive user to autocreate the user account.
...
1. In Opendrive create two new `User Groups` for example: `Proxmox Users` and `Proxmox Admins`.
...
4. Set the `Autocreate Groups` checkbox to have Proxmox automatically create groups based on the groups in Opendrive.
