...
> [!IMPORTANT]
> Cloudflare will need to be able to reach your Opendrive instance and vice versa for this to work correctly
...
6. Paste in the `Client ID` from Opendrive into the `App ID` field.
7. Paste the `Client Secret` from Opendrive into the `Client Secret` field.
8. Paste the `Authorization URL` from Opendrive into the `Auth URL` field.
9. Paste the `Token URL` from Opendrive into the `Token URL` field.
10. Paste the `Certificate URL` from Opendrive into the `Certificate URL` field.
