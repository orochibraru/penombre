...
description: Set up Proxmox Backup Server with Opendrive
---
...
1. In Opendrive create a new OIDC Client, name it, for example, `Proxmox Backup Server`.
...
5. Paste the `Client ID` from Opendrive into the `Client ID` field in PBS.
6. Paste the `Client Secret` from Opendrive into the `Client Key` field in PBS.
...
11. Sign in with the Opendrive account to create the user.
