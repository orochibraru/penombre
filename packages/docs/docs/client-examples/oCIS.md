---
title: oCIS
description: Set up ownCloud Infinite Scale with Opendrive
---
...
- `pocket-id.company.com` is the FQDN of the pocket-id server.
  ...
  > [!NOTE]
  > Opendrive sends the Access-Control-Allow-Origin "pocket-id.company.com" for pocket-id.company.com/.well-known/openid-configuration . See https://github.com/pocket-id/pocket-id/issues/329
  > If you use nginx instead of caddy (CADDY_DISABLED=true) add to the location part of your nginx configuration for Opendrive
...
To support the integration of oCIS with Opendrive, you need to create a OIDC Client in Opendrive.

### Create the groups used by oCIS in Opendrive

1. Log in to Opendrive as an admin
...
### Bind roles used by oCIS with Opendrive groups
...
### Environment Variables for oCIS and OIDC with Opendrive

Add your client id from Opendrive to WEB_OIDC_CLIENT_ID=
...
WEB_OIDC_CLIENT_ID=**<insert your client id from Opendrive>**
...
change line 9 (pocket-id.company.com) under connect-src to your Opendrive URL and mount it to /etc/ocis/csp.yaml in your podman or docker settings.
...
Hardcoded Client secrets are not supported in Opendrive. Given ownCloud have implemented the optional PKCE extension, a workaround is to instead create the ownCloud desktop and mobile clients as public clients.
