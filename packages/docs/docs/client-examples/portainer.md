...
3. Paste the `Client ID` from Opendrive into the `Client ID` field in Portainer.
4. Paste the `Client Secret` from Opendrive into the `Client Secret` field in Portainer.
5. Paste the `Authorization URL` from Opendrive into the `Authorization URL` field in Portainer.
6. Paste the `Token URL` from Opendrive into the `Access token URL` field in Portainer.
7. Paste the `Userinfo URL` from Opendrive into the `Resource URL` field in Portainer.
8. Set the `Redirect URL` to `https://portainer.example.com/`
9. Paste the `Logout URL` from Opendrive into the `Logout URL` field in Portainer.
