---
title: Pingvin Share
description: Set up Pingvin Share with Opendrive OIDC
---
...
1. Create a new OIDC Client in Opendrive (e.g., `pingvin`).
...
2. Scroll down and fill in the fields with values from Opendrive:
...
To control **general** and **admin** access to Pingvin using Opendrive groups:
...
2. Scroll down and fill in the following:
   ...
   - **OpenID Connect role for general access:** `pingvin` (or similar group name from Opendrive)
   - **OpenID Connect role for admin access:** `pingvin_admin` (or similar group name from Opendrive)
