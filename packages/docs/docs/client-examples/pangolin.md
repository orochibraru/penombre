---
title: Pangolin
description: Configure Pangolin with Opendrive authentication
---

## Opendrive Setup

1. In Opendrive create a new OIDC Client, name it, for example, `Pangolin`
...
3. Enter the following:
   ...
   - Authorization URL: Your **Authorization URL** from Opendrive.
   - Token URL: Your **Token URL** from Opendrive.
...
2. Select "External User", select your Opendrive Identity provider, and fill in the relevant details.

Depending on what you configured in `Identifier Path`, you'll need to add that in the `username` field.

- `email` will be your Opendrive email.
- `preferred_username` will be your Opendrive username.
...
Your Callback URL is not correctly defined in Opendrive. Make sure this matches the `Redirect URL` in your Pangolin OIDC config.
