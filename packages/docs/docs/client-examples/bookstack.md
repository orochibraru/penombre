...
1. In Opendrive, create a new OIDC client (example: `bookstack`).
...
BookStack also has the ability to sync OIDC user groups with BookStack roles. By default BookStack will match OIDC groups (Opendrive groups), with the BookStack role display names, with casing ignored.
...
2. Create matching groups in Opendrive:
