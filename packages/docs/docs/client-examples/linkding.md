...
1. **Create a New OIDC Client:**  
   In your Opendrive instance, create a new OIDC Client and give it a name (e.g., "Linkding").
...
4. **Copy Credentials:**  
   Copy the Client ID and Client Secret provided by Opendrive for use in Linkding.
...
1. **Edit Your .env File:**  
   Add the following environment variables to your Linkding `.env` file. Replace the placeholder values with those from Opendrive:

   ```ini
   # Enable OIDC in Linkding
   LD_ENABLE_OIDC=True

   # Client credentials from Opendrive
   OIDC_RP_CLIENT_ID=<your client id from Opendrive>
   OIDC_RP_CLIENT_SECRET=<your client secret from Opendrive>
...
Once redeployed, you should be able to log in using OIDC with Opendrive.
