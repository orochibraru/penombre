---
title: SonarQube
description: Set up SonarQube code analysis with Opendrive
---
...
1. Create a new OIDC Client in Opendrive (e.g., `sonarqube`).
...
To control **admin** access to SonarQube using Opendrive groups:

1. Create your Opendrive group.
...
This will cause SonarQube to automatically add the members of the Opendrive group to the SonarQube `sonar-administrators` administration group.
...
Hint: Thanks to Opendrive `Custom Claims` it is possible to completely manage SonarQube groups via Opendrive.
