---
title: Nextcloud
description: Set up Opendrive authentication for Nextcloud
---
...
- `id.example.com` (The URL of your Opendrive instance.)

## Opendrive Setup

1. In Opendrive create a new OIDC Client, name it, for example, `Nextcloud`.
...
5. Click the `+` and add the information and add the information as follows:
   ...
   2. `Client ID` -> `Client ID`from Opendrive
   3. `Client secret` -> `Client Secret` from Opendrive
   4. `Discovery endpoint` -> `OIDC Discovery URL`from Opendrive
   5. `Custom end session endpoint`-> `Logout URL`from Opendrive
   6. `Scope` -> openid email profile
   7. By default, Nextcloud will create a new user when logging in using Opendrive. If you want to log in with an existing Nextcloud user, you need to tell Nextcloud how to match Opendrive users with Nextcloud users. You can either:
      - Match accounts using a custom claim: `User ID mapping` -> nextcloud_username

        And for each user in the Opendrive configuration, add a custom claim `nextcloud_username` -> the Nextcloud account name to log into.

      - If you have unchecked `Enable Self-Account Editing` in the Opendrive configuration, match accounts using the Opendrive username directly: `User ID mapping` -> preferred_username

        (if `Enable Self-Account Editing` is enabled, Opendrive users are allowed to change their own username and therefore chose the Nextcloud account they log into!)

   8. (Optional) Check `Use group provisioning`if you want Opendrive groups to be replicated on Nextcloud
   9. `Use unique user ID` -> Checked, `Send ID token hint on logout` -> Unchecked

6. After the creation of the Provider, make sure `Backchannel Logout URL` and `Redirect URI` matches the setting on Opendrive.

## Logging In on Nextcloud on mobile (Important)

The mobile app for Nextcloud on IOS does not accept passkey input, which create a small barrier when using Opendrive. If you didn't disable regular login, you can use your Nextcloud username and password to login. If you disabled the regular login, you have to create a `Login Code`on your Opendrive dashboard (id.example.com). After getting the `Login Code`, go to the Nextcloud app, add your Nextcloud URL (nextcloud.example.com). When Opendrive login appears, select `Don't have access to your passkey?`, select `Login Code`and enter the code you received previously.
...
1. Remove built-in login form: You have to execute a command inside the container using Nextcloud CLI method. Since there are many containers and platforms, please make sure use the appropriate form for you container/platform combo
   ...
   2. The built-in login form won't be available anymore when going to `nextcloud.example.com`and will be automatically redirected to login with Opendrive
