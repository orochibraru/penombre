---
title: Harbor
description: Configure Harbor open source registry with Opendrive
---

Below URLs are used as placeholders for the [Harbor](https://goharbor.io/) and Opendrive instances. Replace them with the actual URLs.
...
1. In Opendrive, create a new OIDC client named `Harbor` (or any name you prefer).
...
3. Enter the values for the fields as given below:
   - `Auth Mode` as `OIDC`
   - `Primary Auth Mode` checked if Opendrive should be the primary authentication
   - `OIDC Provider Name` as `Opendrive` or something similar.
   ...
   - `OIDC Admin Group` set to a Opendrive group for administrators (e.g. `admin`) or leave blank
...
- Use `OIDC Group Filter` if not all of your Opendrive user should access the registry.
