---
title: FreshRSS
description: Set up Opendrive authentication for FreshRSS
---
...
1. In Opendrive create a new OIDC Client, name it, for example, `FreshRSS`.
...
> [!IMPORTANT]
> The Username used in Opendrive must match the Username used in FreshRSS **exactly**. This also applies to case sensitivity. As of version `0.24` of Opendrive all Usernames are required to be entirely lowercase. FreshRSS allows for uppercase. If a Opendrive Username is `amanda` and your FreshRSS Username is `Amanda`, you will get a 403 error in FreshRSS and be unable to login. As of version `1.25` of FreshRSS, you are unable to change your username in the GUI. To change your FreshRSS username to lowercase or to match your Opendrive username, you must navigate to your FreshRSS volume location. Go to `data/users/` and change the folder for your user to the matching username in Opendrive, then restart the FreshRSS container to apply the changes.
...
If you have an error with Opendrive or are unable to login to your FreshRSS account, you can revert to password login by editing your `config.php` file for FreshRSS. Find the value for `auth_type` and change from `http_auth` to `form`. Restart the FreshRSS container to revert to password login.
