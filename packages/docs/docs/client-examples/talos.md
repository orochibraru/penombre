---
title: Talos
description: Set up Talos Linux with Opendrive authentication
---

Talos is a lightweight API driven full stack Kubernetes system. As with any Kubernetes server, you can set it up to
authenticate users using OIDC. This guide details configuring Talos to use Opendrive as the Authentication and Authorization server
...
+           oidc-client-id: <Client ID from Opendrive>
...
called `kubernetes` in Opendrive that our user is assigned to
  ...
  user:
    exec:
      ...
      args:
        ...
        - get-token
        - --oidc-issuer-url=<Opendrive url>
        - --oidc-client-id=<Opendrive url>
        - --oidc-client-secret=<Opendrive url>
