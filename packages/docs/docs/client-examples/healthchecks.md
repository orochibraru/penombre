...
4. Create a file beside `docker-compose.yml` called `oauth2-proxy.cfg` with the following config. Make sure to update with your own **Client ID**, **Client Secret**, and **Opendrive URL**:

   ```
   provider_display_name="Opendrive"
   provider="oidc"
   oidc_issuer_url="<<Opendrive URL>>"
   client_id="<<Client ID>>"
...
You can now login to Healthchecks with Opendrive.
