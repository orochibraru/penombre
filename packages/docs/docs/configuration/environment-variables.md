...
Below are all the environment variables supported by Opendrive. These should be configured in your `.env ` file.
...
| `PUID` and `PGID`           | `1000`                                                                                                  | yes                   | The user and group ID of the user who should run Opendrive inside the Docker container and owns the files that are mounted with the volume. You can get the `PUID` and `GUID` of your user on your host machine by using the command `id`. For more information see [this article](https://docs.linuxserver.io/general/understanding-puid-and-pgid/#using-the-variables). |
...
| `PORT`                      | `1411`                                                                                                  | no                    | The port on which Opendrive should listen.                                                                                                                                                                                                                                                                                                                                |
| `HOST`                      | `0.0.0.0`                                                                                               | no                    | The address on which Opendrive should listen.                                                                                                                                                                                                                                                                                                                             |
| `LOG_JSON`                  | `false`                                                                                                 | no                    | If true, emit logs formatted as JSON.                                                                                                                                                                                                                                                                                                                                     |
| `UNIX_SOCKET`               | `-`                                                                                                     | no                    | The Unix socket path on which Opendrive should listen. When set, the server will use a Unix socket instead of TCP, and the `PORT`/`HOST` parameters are ignored.                                                                                                                                                                                                          |
...
| `ANALYTICS_DISABLED`        | `false`                                                                                                 | no                    | Disable heartbeat that gets sent every 24 hours to count how many Opendrive instances are running. Read more [about analytics](/docs/configuration/analytics).                                                                                                                                                                                                            |
...
The `DB_CONNECTION_STRING` environmental variable configures how Opendrive connects to the database.

When using **SQLite** (`DB_PROVIDER=sqlite`, the default), this contains the path to the database, which by default is `data/pocket-id.db`. Opendrive automatically adds some parameters to the database path, turning it into a connection string like `file:data/pocket-id.db?_pragma=journal_mode(WAL)&_pragma=busy_timeout(2500)&_txlock=immediate&_pragma=foreign_keys(1)` - you can pass a full connection string if you need to customize the parameters.
...
We recommend setting an encryption key so Opendrive can encrypt sensitive data, such as the token signing keys. Additionally, providing an encryption key is required when you want to store the token signing keys in the database (`KEYS_STORAGE=database`).
...
You can pass the encryption key to Opendrive in two ways:
...
To enable environment variable overrides, set `UI_CONFIG_DISABLED` to `true`. When `UI_CONFIG_DISABLED` is set to true, Opendrive will use values from the environment variables. If a variable is not set, the system will fall back to its default values.

| Variable                                           | Default Value                | Description                                                                                                                                                                       |
| -------------------------------------------------- | ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `APP_NAME`                                         | `Opendrive`                  | The name of the app.                                                                                                                                                              |
...
| `LDAP_ATTRIBUTE_ADMIN_GROUP`                       | `-`                          | Name of the admin group. Members of this group will have Admin Privileges in Opendrive.                                                                                           |

## Observability

Opendrive offers multiple options for observability, including logs, metrics, and traces.

You can configure Opendrive to emit metrics and traces.
