...
description: Learn how to authenticate OIDC clients in Opendrive using client secrets and federated credentials.
---

In the context of OAuth2 / OpenID Connect, "Clients" refers to applications that request access to protected resources. In Opendrive, they are configured in the _OIDC Clients_ section in the _Settings_ portal.
...
When using OAuth2 with flows such as the "authorization code flow", the most common one for confidential (non-public) clients, in the last step your application exchanges an authorization code for an access token by invoking Opendrive's `/token` endpoint, and including its client ID and secret in the request.
...
> [!NOTE]
> Support for Federated Client Credentials in Opendrive is based on [RFC 7523](https://datatracker.ietf.org/doc/html/rfc7523)
...
## Configuring a client for Federated Client Credentials in Opendrive

When editing a client in Opendrive (in the _OIDC Clients_ section in the _Settings_ page), you can configure Federated Client Credentials for the client.
...
- **Audience** (optional): Must map to the value of the `aud` claim in the JWT tokens.  
  If empty, this defaults to the public URL of Opendrive.
- **Subject** (optional): Must map to the value of the `sub` claim in the JWT tokens.  
  If empty, this defaults to the ID of the OIDC client in Opendrive (the UUID).
...
- **Audience**: Value of the `audience` option specified when creating the Service Account for the Pod. While you can set this to any value, a good option is to use the public URL of Opendrive.
