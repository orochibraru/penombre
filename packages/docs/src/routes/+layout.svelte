...
<svelte:head>
	...
	<meta
		name="description"
		content="Opendrive - A simple OIDC provider that allows users to authenticate with their passkeys to your services."
	/>
...
{#if showBanner}
	<div
		class="sticky top-0 z-[60] border-b border-blue-500 bg-blue-500/10 text-blue-500 backdrop-blur-sm dark:bg-blue-500/15"
	>
		<div class="container-wrapper px-6 py-2">
			<div class="flex items-center justify-center gap-2 text-center text-[12px] font-medium">
				{#if isDev}
					<Code class="size-4" />
					<span
						>Development environment — documentation may not reflect the production
						version</span
					>
				{:else}
					<AlertTriangle class="size-4" />
					<span>
						This documentation is for an unreleased version of Opendrive. See the
						<a
							href="https://pocket-id.org/docs"
							class="font-semibold text-blue-500 underline hover:text-blue-400"
							>latest version</a
						>
						{#if version}
							(v{version}){/if}
