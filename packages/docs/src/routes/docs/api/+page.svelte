...
<div class="max-w-3xl">
    ...
    <p class="mb-6 text-sm leading-relaxed text-(--text-muted)">
        Rendered from the OpenAPI spec checked in at <code
            >static{config.openapi ?? "/openapi.json"}</code
        >, a snapshot taken at build time — "Try it out" here has no live instance behind it unless
        you point it at one of your own.
    </p>
</div>
