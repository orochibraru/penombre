			...
			<div class="flex flex-col gap-2">
				...
				<nav class="relative flex flex-col gap-0.5">
					...
					{#if collapsed && hasMore}
						<div
							class="from-background pointer-events-none absolute bottom-0 left-0 z-10 h-20 w-full bg-gradient-to-t to-transparent"
						></div>
						<Button
							variant="secondary"
							size="sm"
					...
					{#if !collapsed && hasMore}
						<Button
							...
							size="sm"
							class="mt-1 w-full"
							onclick={() => (collapsed = true)}>Show less</Button
						>
					{/if}
				</nav>
			</div>
