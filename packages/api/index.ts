...
async function cleanup() {
	try {
		...
		logger.error("Error during initial cleanup of deleted user storage:", err);
	}
}
		...
		configs: [
			...
			// UI Proxy - serve static files and SSR
			{
				...
				handler: async ({ request }) => {
					...
					const staticPath = `../ui/dist/client${url.pathname}`;
					...
					const uiHandler = await import("../ui/build/handler");
