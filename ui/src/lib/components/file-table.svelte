<script lang="ts">
  ...
  $effect(() => {
    allSelected =
      data.count > 0 && data.list.every((item) => checkedItems[item.key]);
    indeterminate =
      data.count > 0 &&
...
{#snippet tableRow(objectItem: ObjectItem)}
  ...
  {@const isUploading =
    !isFolder && objectItem.size === 0 && !$uploadedItems[objectItem.key]}
    ...
    <Table.Cell class="w-4">
      <Checkbox
        ...
        onCheckedChange={(checked) => {
          checkedItems[item.key] = checked;
          const someChecked = data.list.filter(
              ...
              >
                ...
                {#if item.metadata?.category === "music"}
                  {#if item.metadata.musicduration}
                    ...
                    >
                      {secondsToMinutes(
                        Number.parseFloat(item.metadata.musicduration)
                      )}
                    </Badge>
                  {/if}
                {/if}
  ...
  <AlertDialog.Content class="max-h-[70%] overflow-y-auto pb-16">
    <AlertDialog.Header>
      ...
      <AlertDialog.Description>
        ...
        items from your storage device.
      </AlertDialog.Description>
    </AlertDialog.Header>
    <div class="prose">
      <ul>
